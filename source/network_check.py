...
def MSlistToSet(MS_list):
    final_set = set()
    for i in MS_list:
        final_set.add('MS' + str(i[0]) + '_' + str(i[1]))
    return final_set
...
#    print(visited)
    if last in visited:
        log("Reactant and product are connected.", parameter.path_log) 
        return True
    else:
        log("Reactant and product are NOT connected yet.", parameter.path_log)
        return False
